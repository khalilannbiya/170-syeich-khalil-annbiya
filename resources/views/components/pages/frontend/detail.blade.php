                            ...
                            <div class="flex flex-col gap-2 md:w-full md:flex-row md:flex-wrap">
                                @if ($report->witnesses()->exists())
                                    @foreach ($report->witnesses as $item)
                                        <article class="p-5 bg-white shadow-lg md:basis-[45%]">
                                            <h1 class="text-lg font-semibold capitalize">{{ $item->name }}
                                            </h1>
                                            <p class="mt-1 text-base font-medium">{{ $item->phone }}</p>
                                            <address class="mt-1 text-base">{{ $item->address }}</address>
                                            <p id="description-witness" class="mt-2 text-base font-medium text-pretty">
                                                {{ $item->description }}</p>
                                            <a href="{{ route('reporter.reports.show-witnees-detail', [$item->report_id, $item->id]) }}"
