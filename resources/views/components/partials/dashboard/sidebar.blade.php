        ...
        <nav class="px-4 py-4 mt-5 lg:mt-9 lg:px-6" x-data="{ selected: $persist('Dashboard') }">
            ...
            <div>
                ...
                <ul class="mb-6 flex flex-col gap-1.5">
                    ...
                    <li>
                        <a class="group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark-dashboard dark:hover:bg-meta-4 {{ Route::current()->getName() == strtolower(auth()->user()->role->name) . '.categories.index' ? 'bg-graydark-dashboard dark:bg-meta-4' : '' }}"
                            href="{{ route(strtolower(auth()->user()->role->name) . '.categories.index') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"
                                style="fill: rgba(255, 255, 255, 1);transform: ;msFilter:;">
                                <path
